...
# ---------------- Predict ----------------
def predict_future(model, dataset, sequence_length=7):
    model.eval()
    ...
    # weather - use actual future rain condition if available, otherwise use most recent
    rh = [float(dataset.rain.iloc[len(dataset.rain)-sequence_length + t, 0]) for t in range(sequence_length)]
    rain_history = torch.tensor([rh], dtype=torch.float)           # [1, seq]
    
    # Use the most recent rain condition as future prediction
    future_rain_condition = float(dataset.rain.iloc[-1, 0])
    future_rain = torch.tensor([[future_rain_condition]], dtype=torch.float)  # [1,1]
...
def main():
    ...
    pred, station_keys = predict_future(model, dataset, sequence_length=sequence_length)

    # plot predictions - single figure with all stations
    plt.figure(figsize=(12, 8))
    for i, st in enumerate(station_keys):
        plt.plot(pred[i], label=st)
    
    plt.title('Predicted EV Charging Load')
    plt.xlabel('Hour')
    plt.ylabel('Load')
    plt.grid(True)
    plt.legend()
    ...
    pred_df = pd.DataFrame(pred.T, columns=station_keys)
    pred_df.insert(0, 'Hour', np.arange(24))
    
    pred_df.to_excel('predicted_load.xlsx', index=False)
    print("Predictions saved to predicted_load.xlsx")
